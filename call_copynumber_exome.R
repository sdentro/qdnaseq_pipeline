# This pipeline is suitable for whole exome sequencing
...
load("precalculated_windows/QNDAseq_bins10.RData")
...
p = ggplot(temp) + aes(x=counts, y=..count..) + geom_density()
p2 = ggplot(temp) + aes(x=counts, y=..count..) + geom_histogram(binwidth=5)
png(paste0(samplename, "_control_cov_distribution.png"), height=300, width=600)
grid.arrange(p, p2, ncol=2)
dev.off()

sel = c()
for (chrom in unique(bins$chromosome)) {
	counts_chrom = temp$counts[bins$chromosome==chrom]
	threshold = ifelse(chrom %in% c("X", "Y"), 5, 5)
	print(paste0(chrom, " mean:", mean(counts_chrom), " median:", median(counts_chrom), " sd:", sd(counts_chrom), " threshold:", threshold))
	sel = c(sel, temp$counts[bins$chromosome==chrom] <= (threshold*median(temp$counts[bins$chromosome==chrom], na.rm=T))  & temp$counts[bins$chromosome==chrom] >= (-threshold*median(temp$counts[bins$chromosome==chrom], na.rm=T)))
}
sel[is.na(sel)] = F

print(paste0("Removing bins with excessive control sample counts: ", sum(!sel), " out of", length(sel)))
